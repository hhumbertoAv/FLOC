...
void initialize_results_object(void *identifier, int is_pid)
{
    ...
    if (is_pid) 
    {
        data->identifier.pid = *((int*) identifier);
        data->is_pid = 1;
    } 
    else 
    {
        strncpy(data->identifier.comm_name, (char*) identifier, 255);
        data->identifier.comm_name[255] = '\0'; // Ensure null-termination
        data->is_pid = 0;
    }
...
void write_results(int pid, int time, double power, double energy)
{
    ...
    {
        data->average_power = data->total_energy / data->elapsed_time;
    }
